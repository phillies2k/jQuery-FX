...
/**
 * Including ColorAnimation Plugin for jQuery
 */
...
(function($){
    ...
    var transformPropertyNames          = ['transform', 'WebkitTransform', 'MozTransform', 'OTransform', 'msTransform'],
    ...
    function adjustValue( val, op, now, base ) {
        switch( op ) {
            case '-':
                return parseFloat(op + val);
            case '-=':
                return (now || base) - val;
            case '+=':
                return (now || base) + val;
    ...
    function transformExpressionValue( prop, val, style ) {
        ...
        if (n) {
            ...
            a1 = adjustValue(n[2], n[1], (m ? parseFloat(m[1]) : null), defaults[ prop ]);
            a2 = adjustValue(n[8], n[7], (m ? parseFloat(m[1]) : null), defaults[ prop ]);
